...
def get_place(tankers, types_dict, car_fuel, max_t):
    ...
    k = ''
    for key in qd:
        if qd[key] < max(qd.values()) + 1 and max_t[key] > qd[key]:
            dp = qd[key]
            k = key
    return [dp, k]
    ...
    for key2 in tankers_queue_before:
        for el in tankers_queue_before[key2]:
            if el not in tankers_queue[key2]:
                tm = str(datetime.time(int(str(el).split(':')[0]), int(str(el).split(':')[1])))[:-3]
                print('В  {}  клиент  {} {} {} {}  заправил свой автомобиль и покинул АЗС.'.format(tm, timings_start[el], car_type[timings_start[el]], car_liter[timings_start[el]], car_time[timings_start[el]] ))
                tankers_queue_before[key2].remove(el)
                [print('Автомат №'+key_2+'  максимальная очередь: '+str(tanker_max[key_2])+' Марки бензина: '+' '.join(tanker_fuel[key_2])+' ->'+len(tankers_queue_before[key_2])*'*') for key_2 in tanker_max]
